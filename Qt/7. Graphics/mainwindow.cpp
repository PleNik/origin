...
QVector<uint32_t> MainWindow::ReadFile(QString path, uint8_t numberChannel)
{
    ...
    if(file.isOpen() == false){
        ...
        //не понятно, зачем здесь else. Условие "Если файл не удалось открыть" обработано.
        //Если файл открыт, то код продолжаем. Зачем else?
    }
...
void MainWindow::on_pb_start_clicked()
{
    ...
    auto findMax = [&](QVector<double> res){
                                                ...
                                                */
                                                ...
                                                //устанавливаем шаг сетки и граничные значения
                                                double step = 1/FD;
                                                ...
                                                int steps = round(((maxVal-minVal)/step));
                                                x.resize(steps);
                                                x[0] = minVal;
